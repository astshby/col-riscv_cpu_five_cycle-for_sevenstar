...
module Forwarding_Control_Unit(
    ...
    // -----------------------------------------
    input logic [4:0] rd_addr_MEM,     // MEM 阶段产生的将要写回的目标寄存器
    input logic       reg_write_MEM,   // MEM 阶段是否要写寄存器
    
    input logic [4:0] rd_addr_WB,      // WB 阶段产生的将要写回的目标寄存器
