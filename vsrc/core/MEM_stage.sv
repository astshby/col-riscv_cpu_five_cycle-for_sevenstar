...
module MEM_stage (
    ...
    input logic [`REG_WIDTH-1:0] alu_result,      // MEM阶段访问数据存储器的地址
    input logic [`REG_WIDTH-1:0] rs2_sd_in,   // MEM阶段准备写入的数据 (store_data)

    // 写数据多选，用以产生正确的前递的数据
    input logic [`REG_WIDTH-1:0] PC,
    input logic [`REG_WIDTH-1:0] imm,
    input logic [`RF_WSEL_WIDTH-1:0] reg_w_sel,
    input logic [`REG_WIDTH-1:0] csr_data_old, // CSR 旧值 (RF_WSEL_CSR 时前递)
    output logic [`REG_WIDTH-1:0] forward_data_output, // 真正正确的前递输出 (给 core_top 里的 Forwarding MUX)

    // load/store 数据前递相关输入
    input logic forward_data_sel_MEM,
    input logic [`REG_WIDTH-1:0] wb_data, // 来自 WB 阶段的前递数据
    ...
    // 处理 Store 指令在 MEM 阶段的写入数据前递 (Load -> Store )
    logic [`REG_WIDTH-1:0] rs2_data_fwd;
    assign rs2_data_fwd = forward_data_sel_MEM ? wb_data : rs2_sd_in;

    // 存储器接口 — Store 逻辑 (mem_mux_s，Load 提取已移至 WB 的 mem_mux_l)
    mem_mux_s mem_mux_s(
        .mem_re(mem_re),
        .mem_we(mem_we),
        .funct3(funct3),
